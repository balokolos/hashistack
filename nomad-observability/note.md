problem:
data directory permission. task cannot write. permission denied. chmod 777 to data dir
docker sock issue. add docker plugin to nomad.hcl
grafana page not found. need to use https as define in traefic tag.
grafana cannot add loki/prometheus. Need to check



Prerequisite:
nomad config in nomad.hcl need to define data mount and enable host docker 
...
Yes. Below is a lab-ready Nomad + Consul monitoring stack. I’ll keep the first version deliberately simple: single-instance Grafana, Prometheus and Loki, Traefik in front, and Alloy/node-exporter on each Nomad client.
The architecture is:
                         ┌─────────────────┐
                         │      Users      │
                         └────────┬────────┘
